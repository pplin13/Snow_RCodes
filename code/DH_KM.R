...
timeseries1 <- matrix(data = NA, nrow = length(zDH_1), ncol = 2)
timeseries1[,1] <- zDH_1
timeseries1[,2] <- DH_1

timeseries2 <- matrix(data = NA, nrow = length(zDH_2), ncol = 2)
timeseries2[,1] <- zDH_2
timeseries2[,2] <- DH_2

timeseries3 <- matrix(data = NA, nrow = length(zDH_3), ncol = 2)
timeseries3[,1] <- zDH_3
timeseries3[,2] <- DH_3

timeseries4 <- matrix(data = NA, nrow = length(zDH_4), ncol = 2)
timeseries4[,1] <- zDH_4
timeseries4[,2] <- DH_4

timeseries5 <- matrix(data = NA, nrow = length(zDH_5), ncol = 2)
timeseries5[,1] <- zDH_5
timeseries5[,2] <- DH_5
...
amin <- min(c(DH_1,DH_2,DH_3,DH_4,DH_5))
amax <- max(c(DH_1,DH_2,DH_3,DH_4,DH_5))

#source("ddj2.R")
# Estimate the drift, diffusion and jump parameters non-parametrically
ddjDH1 <- ddj(timeseries1, bandwidth=bw, na=na, dt = dz, amin, amax)
ddjDH2 <- ddj(timeseries2, bandwidth=bw, na=na, dt = dz, amin, amax)
ddjDH3 <- ddj(timeseries3, bandwidth=bw, na=na, dt = dz, amin, amax)
ddjDH4 <- ddj(timeseries4, bandwidth=bw, na=na, dt = dz, amin, amax)
ddjDH5 <- ddj(timeseries5, bandwidth=bw, na=na, dt = dz, amin, amax)

Rbin_DH <- Rbin <- ddjDH1$avec

D1mat <- cbind(ddjDH1$D1, ddjDH2$D1, ddjDH3$D1, ddjDH4$D1, ddjDH5$D1)
D1_DH <- D1 <- rowMeans(D1mat)
eD1_DH <- eD1 <- rowSds(D1mat)/sqrt(6)

K2mat <- cbind(ddjDH1$K2, ddjDH2$K2, ddjDH3$K2, ddjDH4$K2, ddjDH5$K2)
K2_DH <- K2 <- rowMeans(K2mat)
eK2_DH <- eK2 <- rowSds(K2mat)/sqrt(6)

K4mat <- cbind(ddjDH1$K4, ddjDH2$K4, ddjDH3$K4, ddjDH4$K4, ddjDH5$K4)
K4_DH <- K4 <- rowMeans(K4mat)
eK4_DH <- eK4 <- rowSds(K4mat)/sqrt(6)

K6mat <- cbind(ddjDH1$K6, ddjDH2$K6, ddjDH3$K6, ddjDH4$K6, ddjDH5$K6)
K6_DH <- K6 <- rowMeans(K6mat)
eK6_DH <- eK6 <- rowSds(K6mat)/sqrt(6)

sigma2_mat <- K6mat/(5*K4mat)
sigma2_DH <- sigma2 <- K6/(5*K4)
#esigma2_DH <- esigma2 <- sigma2*sqrt((eK6/K6)^2 + (eK4/K4)^2)
esigma2_DH <- esigma2 <- rowSds(sigma2_mat)/sqrt(6)
...
lambda_DH <- lambda <- ifelse(lambda < (1/dz), lambda, 1/dz)
elambda_DH <- elambda <- ifelse(elambda < (1/dz), elambda, 1/dz)

D2mat <- K2mat - lambda_mat*sigma2_mat
D2 <- K2 - lambda*sigma2
#eD2_DH <- eD2 <- sqrt(eK2^2 + (eK4*(10/3)*(K4/K6))^2 + (eK6*(5/3)*(K4/K6)^2)^2)
eD2_DH <- eD2 <- rowSds(D2mat)/sqrt(6)
D2_DH <- D2 <- ifelse(D2>0,D2,0)

DiffJump_mat <- D2/(lambda_mat*sigma2_mat)
DiffJump_DH <- DiffJump <- D2/(lambda*sigma2)
#eDiffJump_DH <- eDiffJump <- sqrt((eK2*(3/5)*(K6/K4^2))^2 + (eK4*(6/5)*(K2*K6/K4^3))^2 + (eK6*(3/5)*(K2/K4^2))^2)
eDiffJump_DH <- eDiffJump <- rowSds(DiffJump_mat)/sqrt(6)
...
pdf('plots/D1_DH_.pdf',width = 4, height = 3.5)
...
pdf('plots/D2_DH_.pdf',width = 4, height = 3.5)
par(mfrow=c(1,1),mar=c(4, 5, 0, 0),mgp=c(3,1,0),oma=c(1,1,1,1))
...
pdf('plots/K4_DH_.pdf',width = 4, height = 3.5)
par(mfrow=c(1,1),mar=c(4, 5, 0, 0),mgp=c(3,1,0),oma=c(1,1,1,1))
...
pdf('plots/JumpProb_DH_.pdf',width = 4, height = 3.5)
par(mfrow=c(1,1),mar=c(4, 5, 0, 0),mgp=c(3,1,0),oma=c(1,1,1,1))
...
pdf('plots/JumpAmp_DH_.pdf',width = 4, height = 3.5)
par(mfrow=c(1,1),mar=c(4, 5, 0, 0),mgp=c(3,1,0),oma=c(1,1,1,1))
...
pdf('plots/DiffJump_DH_.pdf',width = 4, height = 3.5)
par(mfrow=c(1,1),mar=c(4, 5, 0, 0),mgp=c(3,1,0),oma=c(1,1,1,1))
plot(Rbin,DiffJump,type='p',lwd=1,col='black',
     ...
     cex.lab = 1.3, cex.axis=1.3, xlim = c(-4,4), ylim = c(0,1.5)) #max(D2sig2_DH)))
